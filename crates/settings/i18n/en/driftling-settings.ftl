...
sync-mode-off = This computer only
sync-mode-server = My server
sync-mode-folder = Shared folder
...
sync-folder = Folder
sync-folder-hint = A directory that is synchronised between computers
sync-folder-empty = Pick a folder
sync-folder-missing = No such folder
sync-folder-not-dir = That is not a folder
sync-off-hint = The pet lives here only; the journal goes nowhere
