...
sync-mode-off = Только этот компьютер
sync-mode-server = Мой сервер
sync-mode-folder = Общая папка
...
sync-folder = Папка
sync-folder-hint = Каталог, который синхронизируется между компьютерами
sync-folder-empty = Укажите папку
sync-folder-missing = Такой папки нет
sync-folder-not-dir = Это не папка
sync-off-hint = Питомец живёт только здесь; журнал никуда не уходит
