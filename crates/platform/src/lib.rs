...
pub enum Event {
    ...
    /// Список выходов в ГЛОБАЛЬНЫХ логических координатах, слева направо,
    /// и индекс того, на котором сейчас живёт оверлей. Приходит при старте
    /// и при каждом изменении набора мониторов.
    ///
    /// Нужен, чтобы питомец мог уйти на соседний монитор: только платформа
    /// знает, есть ли там что-то справа и где именно.
    Outputs {
        rects: Vec<Rect>,
        current: usize,
    },
...
pub trait App {
    ...
    /// Приложение просит переехать на другой выход (индекс из
    /// [`Event::Outputs`]). Бэкенд спрашивает раз в кадр и, переехав,
    /// присылает новую [`Event::OutputGeometry`].
    ///
    /// Возврат забирает намерение: спросили — значит взяли в работу.
