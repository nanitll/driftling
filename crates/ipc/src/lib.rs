...
pub enum Request {
    ...
    /// Патч в боксе: он на порядок больше любого другого запроса, и без
    /// бокса каждый `Request` в очереди занимал бы его размер.
    SetConfig {
        patch: Box<driftling_core::ConfigPatch>,
    },
