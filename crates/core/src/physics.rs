...
impl Surface {
    ...
    ///
    /// На стене питомец виден В ПРОФИЛЬ и держится за неё мордой: кадры
    /// `climb`/`cling` нарисованы сбоку, повороты тут не нужны — нужно
    /// только зеркало, чтобы он смотрел на свою стену. Направление
    ...
    pub fn orient(self, facing: Direction) -> Orient {
        ...
        match self {
            ...
            Surface::WallRight => Orient::IDENTITY,
        }
    }
