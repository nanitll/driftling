...
impl Surface {
    ...
    ///
    /// На полу — привычное зеркало по направлению взгляда.
    ///
    /// На стене питомец лезет БОКОМ: кадр поворачивается на четверть, ноги
    /// упираются в стену, морда остаётся видна. Зеркало (по направлению
    /// движения) после поворота работает вдоль стены — «нос» ведёт шаг.
    ///
    ...
    pub fn orient(self, facing: Direction) -> Orient {
        ...
        match self {
            ...
            // Поворот на четверть по часовой: низ кадра уходит влево — ноги
            // в левую стену. Против часовой (три четверти) — в правую.
