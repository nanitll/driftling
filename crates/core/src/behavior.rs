...
pub struct BehaviorConfig {
    ...
    // ---- Укачивание (фаза G5, баланс G6) ----
    /// Скорость руки, ниже которой движение вообще не считается тряской, м/с.
    pub shake_speed_mps: f32,
    /// Сколько укачивания даёт один разворот на пороговой скорости
    /// (быстрее — до втрое больше).
    pub shake_gain: f32,
    /// Насколько косинус угла между ходами считается разворотом:
    /// -0.3 — это поворот круче ~110°, плавную дугу так не поймать.
    pub shake_reverse_dot: f32,
    /// Скорость, с которой укачивание проходит вне руки, ед/с.
    pub nausea_fade: f32,
    ...
    /// Порог покоя формы: ближе этого к единице тело считается ровным и
    /// не деформируется вовсе. Без него пиксельный спрайт «кипит» — при
    /// целочисленном масштабе даже полпроцента видно как дрожь.
    pub squash_deadzone: f32,
...
impl Default for BehaviorConfig {
    fn default() -> Self {
        Self {
            ...
            // Баланс: чтобы позеленел, надо трясти всерьёз секунды три,
            // до тошноты — секунд шесть-семь; обычный перенос не считается.
            shake_speed_mps: 2.0,
            shake_gain: 0.013,
            ...
            // качками и успокаивается, а не звенит.
            squash_damping: 18.0,
            // Затухание подняли — импульс удара поднимаем следом, иначе
            // сминание стало бы едва заметным.
            squash_impact: 0.9,
            stretch_in_air: 0.22,
            squash_deadzone: 0.025,
            lean_spring: 120.0,
            lean_damping: 12.0,
        }
    }
