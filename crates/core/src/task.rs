...
//! дойти до лежанки и лечь, догнать мяч. Всё, что питомец
...
//!   питомец сходил за мячом, в ней не нужно.
...
pub struct Errand {
    pub kind: ErrandKind,
    /// Предмет, ради которого всё затевалось (лужа, лежанка, мяч).
    pub prop: Option<u64>,
...
mod tests {
    ...
    fn errand_faces_the_way_it_walks() {
        ...
        let mut errand = Errand::new(ErrandKind::Nap, 100.0, 0.5);
